...
If I had more time I would have continued exploring more variables. Developed versus developing country, overall GDP amount, and economic systems would all be interesting to analyze in relation to corruption and natural resources. I am also skeptical that a linear model is truly the correct model to be using in this situation, as corruption is very complex and probably not a completely linear issue. 

```{r}
geo <- read_csv(here("data", "world_country_lat_long.csv")) |> 
  select(country_code, latitude, longitude, country)

geo_join <- full_join(corrupt_resource_select, geo, by = c("country" = "country")) 
geo_join <- geo_join[1:153, ] |> 
  filter(!is.na(latitude))
```

```{r}
geo_lm <- lm(cpi_score_2021 ~ total_natural_resources_rents_percent_of_gdp, data = geo_join)

geo_join$residual <- resid(geo_lm)
coords <- as.matrix(geo_join[, c("longitude", "latitude")]) 
geo_join.nb <- dnearneigh(coords, d1 = 0, d2 = 50)
geo_join.lw <- nb2listw(geo_join.nb, style = "W", zero.policy = FALSE)
inc.lag <- lag.listw(geo_join.lw, geo_join$residual)
tibble(residual = geo_join$residual, neighborhood = inc.lag) |> 
  ggplot(aes(residual, neighborhood)) +
  geom_point() 
```
...
moran_test <- moran.test(geo_join$residual, geo_join.lw, zero.policy = TRUE)

# Print the Moran's I test results
print(moran_test)

observed_morans_i <- moran_test$estimate["Moran I"]
print(observed_morans_i)
```
...
for (i in 1:n_permutations) {
  geo_join$permuted_cpi <- sample(geo_join$cpi_score_2021)
  permuted_model <- lm(permuted_cpi ~ total_natural_resources_rents_percent_of_gdp, data = geo_join)
  null_distribution[i] <- coef(permuted_model)[2]  # Slope of the model
}

library(ggplot2)

null_dist_df <- data.frame(null_distribution)

ggplot(null_dist_df, aes(x = null_distribution)) +
  geom_histogram(binwidth = 0.01, color = "black", fill = "skyblue") +
 geom_vline(xintercept = observed_morans_i, color = "red", linetype = "dotted") +
  labs(
    title = "Null Distribution of the Slope",
    x = "Slope under the Null Hypothesis",
    y = "Frequency"
  )
...
```{r}
geo_join.nb <- dnearneigh(coords, d1 = 0, d2 = 50)
geo_join.lw <- nb2listw(geo_join.nb, style = "W")
sperr_mod <- spatialreg::errorsarlm(cpi_score_2021 ~ total_natural_resources_rents_percent_of_gdp,
                        data = geo_join,
                        listw = geo_join.lw)
summary(sperr_mod)
