...
When we look at the residuals of our original linear model, we see that they are clumped on the far right side of the graph. I believe that indicates where there is a large clump of data points around the less than 2% GDP mark. 
...
Let's look at what countries are in that clump

```{r}
low_percent <- corrupt_resource_select |> 
  filter(total_natural_resources_rents_percent_of_gdp < 2)
print(dim(low_percent))
```

There are 75 countries that have a GDP percentage under 2 from natural resources. They are not geographically related, Europe has 36 countries, Asia has 17, Africa has 8, the Americas have 12, Oceania has 2. They are not all developed or undeveloped, 29 are developed and 46 are developing. Without more time, we can't fully examine the relationship between the countries in this cluster.
